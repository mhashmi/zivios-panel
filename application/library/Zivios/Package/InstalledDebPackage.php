...
class Zivios_Package_InstalledDebPackage extends Zend_Db_Table
{
    protected $_name = 'inst_deb_packages';
    protected $_id = 'id';
    ...
    {
        $sql = "select * from inst_deb_packages where name=:name and emscomputerdn=:emsdn";
    ...
    {
        $sql = "select id from inst_deb_packages where emscomputerdn='".$computerdn."' order by name";
    ...
    public function searchAllForComputer($computerdn,$name,$limit=100000,$offset=0)
    {
        ...
        $sql = "select id from inst_deb_packages where emscomputerdn='".$computerdn."' "
    ...
    {
        $sql = "select * from inst_deb_packages where id=".$id;
        $rows = $this->_db->fetchRow($sql);

        Zivios_Util::autoPopulateFromRows($this,$rows);
    }

    public function setData($name,$short_desc,$long_desc,$version,$status,$conflicts,$replaces,$depends)
    {
        $this->name = $name;
